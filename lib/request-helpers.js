...
export const getDailyDistinctCountQuery = (table, column, asColumn = null) => {
  if (!asColumn) asColumn = column + '_count'
  return `
    SELECT day::TEXT, COUNT(DISTINCT ${column})::INT as ${asColumn}
    FROM ${table}
...
export const getMonthlyDistinctCountQuery = (table, column, asColumn = null) => {
  if (!asColumn) asColumn = column + '_count'
  return `
    SELECT
      date_trunc('month', day)::DATE::TEXT as month,
      COUNT(DISTINCT ${column})::INT as ${asColumn}
    FROM ${table}
    WHERE
